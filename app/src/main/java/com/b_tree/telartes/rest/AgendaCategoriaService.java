 ...
 */
public abstract class AgendaCategoriaService  extends  ClasesRest{

    private List<Categoria> listaCategorias;

    public AgendaCategoriaService(Context context) {
        super(context);
    ...
    protected String getMetod() {
        // TODO Auto-generated method stub
        return "categoriaculturalrest";
    }


    @Override
    public void onFailure(int i, Header[] headers, byte[] bytes, Throwable throwable) {
        Log.d("ERROR", "no consume categorias");
    }
    ...
    public void onSuccess(int i, Header[] headers, byte[] bytes) {
        try {
            ...
            for (int c = 0; c <jsonArray.length() ; c++) {
                listaCategorias.add(new Categoria(jsonArray.getJSONObject(c),"Agenda"));
            }
